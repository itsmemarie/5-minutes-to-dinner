...
import { C, ep, mn, screenTitle, R } from '../lib/theme.js'
      ...
      {DAYS.map(day => {
        ...
        return (
          <div key={day} style={{ marginBottom:16 }}>
            ...
            <div onDragOver={e=>onDragOver(e,day)} onDragLeave={onDragLeave} onDrop={e=>onDrop(e,day)} style={{outline:over===day&&drag?.fromDay!==day?`2px solid ${C.primary}`:'2px solid transparent',borderRadius:R.md,transition:'outline 0.12s'}}>
              {meals.length === 0 ? (
                <button onClick={()=>onDayOpen(day)} style={{width:'100%',background:C.primaryFixed,border:'none',borderRadius:R.md,padding:'16px',...mn,fontSize:13,color:C.onSurfaceVariant,fontStyle:'italic',cursor:'pointer',textAlign:'left'}}>+ Add a meal</button>
              ) : meals.map(m => (
                <button key={m.id} draggable onDragStart={e=>onDragStart(e,m.id,day,m.section)} onDragEnd={onDragEnd} onClick={()=>onRecipeOpen(m.recipeId, m.portion)} style={{width:'100%',background:C.primaryFixed,border:'none',borderRadius:R.md,padding:'14px 16px',marginBottom:8,display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,cursor:drag?'grabbing':'grab',textAlign:'left',opacity:drag?.mealId===m.id?0.45:1,transition:'opacity 0.15s'}}>
                  <div style={{minWidth:0}}>
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap',rowGap:4}}>
                      <span style={{...mn,fontSize:11,fontWeight:600,background:C.white,color:C.onSurface,padding:'3px 10px',borderRadius:R.pill}}>{SEC_LBL[m.section]}</span>
