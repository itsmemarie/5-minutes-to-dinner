    ...
    <div onClick={disabled?undefined:()=>onToggle(r.id)} style={{...CARD,background:C.surfaceContainerHigh,padding:'12px 14px',display:'flex',alignItems:'center',gap:12,cursor:disabled?'default':'pointer',opacity:disabled?0.55:1,marginBottom:8}}>
      <div style={{flex:1}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
          ...
          {r.fun&&<span style={{...mn,fontSize:10,background:C.secondaryContainer,color:C.onSecondaryContainer,padding:'1px 6px',borderRadius:R.pill,fontWeight:700}}>F</span>}
          {r.husband&&<span style={{...mn,fontSize:10,background:C.secondaryContainer,color:C.onSecondaryContainer,padding:'1px 6px',borderRadius:R.pill,fontWeight:700}}>H</span>}
        </div>
      </div>
      <div style={{width:30,height:30,borderRadius:R.pill,border:`2px solid ${disabled?C.outlineVariant:sel?C.primary:C.outlineVariant}`,background:disabled?C.surfaceContainerHigh:sel?C.primary:'transparent',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,color:disabled?C.outline:sel?C.onPrimary:C.outline,fontSize:14,fontWeight:700}}>
        {disabled?'✓':sel?'✓':'+'}
      </div>
    </div>
  )
