    ...
    try {
      ...
      DAYS.forEach(day => {
        ['breakfast','main','side'].forEach(sec => {
          plan[day][sec].forEach(m => allMeals.push({ day, section: sec, name: m.name, portion: m.portion }))
        })
      })
      if (!allMeals.length) { setNutriError('Add some meals to the planner first.'); setNutriLoading(false); return }
      // Call for both profiles in parallel
      const [adultResult, toddlerResult] = await Promise.all([
        callEdgeFn('nutrition', { profile: 'adult',         meals: allMeals, weekOf: WEEK_OF, region: 'UK' }),
        callEdgeFn('nutrition', { profile: 'toddler_18mo',  meals: allMeals, weekOf: WEEK_OF, region: 'UK' }),
      ])
