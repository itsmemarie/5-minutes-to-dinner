...
function RecipeSelectionScreen({day,section,plan,recipes,onAdd}){
  ...
  const currentIds=useMemo(()=>[...plan[day].breakfast,...plan[day].main,...plan[day].side].map(m=>m.recipeId),[plan,day])
    ...
    <div style={{display:'flex',flexDirection:'column',minHeight:'100%'}}>
      <div style={{flex:1,padding:'12px 20px 120px'}}>
        ...
        {chip==='freezer'?(
          ...
          <>
            <RecipeBucket title='Already Planned' items={already} disabled={true} selected={selected} onToggle={toggle}/>
            <RecipeBucket title='Default Meals' items={defaults} disabled={false} selected={selected} onToggle={toggle}/>
...
// ─── Nutrition ────────────────────────────────────────────────────────
function NutritionScreen({profile,setProfile}){
  const data=NUTRI[profile]
...
// ─── Shopping List ────────────────────────────────────────────────────
function ShoppingListScreen({shopping,onToggle}){
  const AISLES=['VEGETABLES','MEAT','DAIRY','DRY GOODS','FRUIT','PANTRY','OTHER']
  ...
  return(
    <div style={{padding:'16px 20px 20px'}}>
      ...
      <div style={{...mn,fontSize:12,color:C.primary,marginTop:2,fontWeight:600,marginBottom:12}}>{unc} item{unc!==1?'s':''} remaining</div>
      <HDivider/>
      {shopping.length===0?(
        <div style={{...CARD,padding:32,textAlign:'center'}}>
          <div style={{fontSize:32,marginBottom:12}}>🛒</div>
          <p style={{...mn,fontSize:13,color:C.onSurfaceVariant,lineHeight:1.6}}>No shopping list yet. Go to Planner and tap Generate Shopping List.</p>
        </div>
      ):(
        AISLES.map(aisle=>{
...
export default function App() {
  ...
  const generateShoppingList = async () => {
    const aisleFor = sec => sec === 'main' ? 'MEAT' : sec === 'side' ? 'VEGETABLES' : 'OTHER'
    const items = []
    DAYS.forEach(day => ['breakfast', 'main', 'side'].forEach(sec => {
      plan[day][sec].forEach(m => {
        const ex = items.find(i => i.name === m.name)
        if (ex) { ex.amount += m.portion } else { items.push({ name: m.name, amount: m.portion, unit: 'portions', aisle: aisleFor(sec), checked: false }) }
      })
    }))
    if (!items.length) { alert('Add some meals to the planner first.'); return }
  ...
  const renderScreen = () => {
    ...
    if (screen === 'nutrition')       return <NutritionScreen profile={nutriProf} setProfile={setNutriProf}/>
    ...
    if (tab === 'planner') return <PlannerScreen plan={plan} removeMeal={removeMeal} onDayOpen={openDayPlan} onNutrition={()=>setScreen('nutrition')} onShoppingList={generateShoppingList}/>
    if (tab === 'list')    return <ShoppingListScreen shopping={shopping} onToggle={onShoppingToggle}/>
    if (tab === 'batch')   return <BatchScreen activeTab={batchTab} setActiveTab={setBatchTab}/>
  }
